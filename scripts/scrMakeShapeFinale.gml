///scrMakeShapeFinale(x, y, object, sides, speed, angle, distance, number_per_side, make_star, star_pointiness, make_flower)
var xx = argument[0];
var yy = argument[1];
...
//var parent_objectID = argument[11];
//var tagID = argument[12];
...
for (var i = 0; i < sides; i++) {
    ...
    for (var j = 0; j < checkDistance; j += checkDistance / argument[7]) {
        ...
        s . direction = radtodeg(centerAngle);
        s . speed = argument[4];
        s . image_blend = make_colour_hsv(map(s.direction, 0, 359, 0, 255),180,255);
